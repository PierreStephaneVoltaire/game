...
variable "entra_admin_type" {
  description = "Type of the active Terraform identity used as PostgreSQL administrator."
  type        = string
  default     = "ServicePrincipal"

  validation {
    condition     = contains(["User", "ServicePrincipal"], var.entra_admin_type)
    error_message = "Use User or ServicePrincipal to match the active Terraform identity."
  }
}
